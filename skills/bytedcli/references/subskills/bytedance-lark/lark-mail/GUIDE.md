...
- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。需要先通过 `bytedcli lark auth login --domain mail` 完成用户授权。
- **`--as bot`**：以应用身份访问邮箱。需要在飞书开发者后台为应用开通相应权限，否则请求会被拒绝。**注意：bot 身份仅适用于读取类操作，所有写操作（发送、回复、转发、草稿编辑等）仅支持 user 身份。**

1. 所有邮件写操作（发送、回复、转发、草稿编辑） → 必须使用 `--as user`，未登录时先使用 `bytedcli lark auth login --domain mail` 进行登录
2. 读取类操作（查看邮件、会话、收件箱列表等） → 推荐使用 `--as user`；如需应用级批量读取（如管理员代操作），可使用 `--as bot`，确保应用已开通对应权限

## 典型工作流

1. **确认身份** — 首次操作邮箱前先调用 `bytedcli lark mail user_mailboxes profile --params '{"user_mailbox_id":"me"}'` 获取当前用户的真实邮箱地址（`primary_email_address`），不要通过系统用户名猜测。后续判断"发件人是否为用户本人"时以此地址为准。
...
```bash
# Shortcut
bytedcli lark mail +triage -h
bytedcli lark mail +send -h

# 原生 API（逐级查看）
bytedcli lark mail user_mailbox.messages -h
```
...
```bash
bytedcli lark mail multi_entity search --as user --data '{"query":"<关键词>"}'
```
...
```bash
# 查询可访问的邮箱（主邮箱 + 公共邮箱）
bytedcli lark mail user_mailboxes accessible_mailboxes --params '{"user_mailbox_id":"me"}'

# 查询某个邮箱的可用发信地址（主地址、别名、邮件组）
bytedcli lark mail user_mailbox.settings send_as --params '{"user_mailbox_id":"me"}'
```
...
bytedcli lark mail +send --mailbox shared@example.com \
...
bytedcli lark mail +send --mailbox me --from alias@example.com \
...
```bash
bytedcli lark mail user_mailbox.messages send_status --params '{"user_mailbox_id":"me","message_id":"<发送返回的 message_id>"}'
```
...
```bash
bytedcli lark mail user_mailbox.drafts cancel_scheduled_send --params '{"user_mailbox_id":"me","draft_id":"<draft_id>"}'
```
...
**撤回操作：**
```bash
bytedcli lark mail user_mailbox.sent_messages recall --as user \
...
**查询撤回进度：**
```bash
bytedcli lark mail user_mailbox.sent_messages get_recall_detail --as user \
  --params '{"user_mailbox_id":"me","message_id":"<message_id>"}'
```
...
1. 分享单封邮件到群聊（默认 `--receive-id-type chat_id`）：
   ```bash
   bytedcli lark mail +share-to-chat --message-id <邮件ID> --receive-id oc_xxx
   ```

2. 分享整个会话到群聊：
   ```bash
   bytedcli lark mail +share-to-chat --thread-id <会话ID> --receive-id oc_xxx
   ```

3. 通过邮箱分享给个人：
   ```bash
   bytedcli lark mail +share-to-chat --message-id <邮件ID> --receive-id user@example.com --receive-id-type email
   ```

4. 如果不知道群聊 ID，先搜索：
   ```bash
   bytedcli lark im +chat-search --query "群名关键词"
   ```
...
bytedcli lark mail +send --as user \
...
```bash
# ✅ 推荐：HTML 格式
bytedcli lark mail +send --to alice@example.com --subject '周报' \
  --body '<p>本周进展：</p><ul><li>完成 A 模块</li><li>修复 3 个 bug</li></ul>'

# ⚠️ 仅在内容极简时使用纯文本
bytedcli lark mail +reply --message-id <id> --body '收到，谢谢'
```
...
```bash
# ✅ 验证操作结果：不需要 HTML
bytedcli lark mail +message --message-id <id> --html=false

# ✅ 需要阅读完整内容：保持默认
bytedcli lark mail +message --message-id <id>
```
...
- 列表 / 获取 / 删除 走原生 API：`bytedcli lark mail user_mailbox.templates {list|get|delete} ...`。
...
```bash
bytedcli lark schema mail.<resource>.<method>
# 例如：bytedcli lark schema mail.user_mailbox.messages.modify_message
```

> **⚠️ 注意**：① 必须精确到 method 级别，禁止查 resource 级别（如 `bytedcli lark schema mail.user_mailbox.messages`，输出 78K）。② schema 路径用 `.` 分隔（`mail.user_mailbox.messages.modify_message`），但 CLI 命令在 resource 和 method 之间用**空格**（`bytedcli lark mail user_mailbox.messages modify_message`），不要混淆。
...
```
bytedcli lark mail <resource> <method> --params '{...}' [--data '{...}']
```
...
bytedcli lark mail user_mailbox.messages list \
...
bytedcli lark mail user_mailbox.folders create \
...
Shortcut 是对常用操作的高级封装（`bytedcli lark mail +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema mail.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark mail <resource> <method> [flags] # 调用 API
```
