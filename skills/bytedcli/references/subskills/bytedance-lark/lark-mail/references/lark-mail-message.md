...
本 skill 对应 shortcut `bytedcli lark mail +message`，内部步骤：
...
```bash
# 读取一封邮件（默认包含 HTML 正文）
bytedcli lark mail +message --message-id <message-id>

# 仅纯文本正文（更小的负载，适合 AI 处理）
bytedcli lark mail +message --message-id <message-id> --html=false

# 指定邮箱
bytedcli lark mail +message --mailbox user@example.com --message-id <message-id>

# JSON 输出（脚本友好）
bytedcli lark mail +message --message-id <message-id> --format json

# Dry Run
bytedcli lark mail +message --message-id <message-id> --dry-run
```
...
bytedcli lark mail +message --message-id <id> --format json | jq -r '.data.body_html'
```
...
```bash
# 1. 读取邮件（仅纯文本，更小负载）
bytedcli lark mail +message --message-id <id> --html=false --format json

# 2. 让 LLM 分析 body_plain_text 并起草回复

# 3. 发送回复
bytedcli lark mail +reply --message-id <id> --body "..."
```
...
bytedcli lark mail +message --message-id <id> --format json

# 2. 仅为需要的 ID 获取下载 URL
bytedcli lark schema mail.user_mailbox.message.attachments.download_url
bytedcli lark mail user_mailbox.message.attachments download_url \
...
- `bytedcli lark mail +thread` — 读取会话中所有邮件
- `bytedcli lark mail +reply` — 回复邮件
- `bytedcli lark mail +forward` — 转发邮件
- `bytedcli lark mail user_mailbox.message.attachments download_url` — 按需获取邮件附件/图片下载 URL
- `bytedcli lark mail user_mailbox.messages list` — 列出收件箱邮件（获取 `message_id`）
