...
本 skill 对应 shortcut：`bytedcli lark mail +signature`。

## 命令

```bash
# 列出所有签名
bytedcli lark mail +signature

# 查看某个签名的详情（渲染后的内容预览、模板变量值、图片信息）
bytedcli lark mail +signature --detail <signature_id>

# 指定邮箱
bytedcli lark mail +signature --from shared@example.com
```
...
# 在发送邮件时附加签名
bytedcli lark mail +send --to alice@example.com --subject '你好' --body '<p>内容</p>' --signature-id <签名ID>
```
