...
本 skill 对应 shortcut：`bytedcli lark mail +send-receipt`。
...
bytedcli lark mail +send-receipt --message-id <message-id> --yes

# 指定邮箱（公共邮箱场景）
bytedcli lark mail +send-receipt --mailbox shared@example.com --message-id <message-id> --yes

# Dry Run（不真发）
bytedcli lark mail +send-receipt --message-id <message-id> --dry-run
```
...
```bash
# 1. 拉信
bytedcli lark mail +message --message-id msg-1 --format json | jq '.data.label_ids'
...
# 3. 用户确认后发回执
bytedcli lark mail +send-receipt --message-id msg-1 --yes
```
...
```bash
# 1. 筛出带 -607 标签的邮件
bytedcli lark mail +triage --folder INBOX --format json \
...
bytedcli lark mail +send-receipt --mailbox support@example.com --message-id <id> --yes
```
...
- `bytedcli lark mail +message` — 拉单封邮件（在 `label_ids` 里检查 `READ_RECEIPT_REQUEST`）
- `bytedcli lark mail +send --request-receipt` — 反向：**请求**别人回执
- `bytedcli lark mail user_mailbox.messages send_status` — 查询回执邮件的投递状态
