...
bytedcli lark mail +triage

# 查看收件箱未读
bytedcli lark mail +triage --filter '{"folder":"inbox","is_unread":true}'

# 全文搜索
bytedcli lark mail +triage --query "合同审批"

# 按发件人 / 主题搜索
bytedcli lark mail +triage --filter '{"from":["boss@example.com"],"subject":"季度报告"}'

# 按时间范围搜索（如"上周的邮件"）
bytedcli lark mail +triage --query "项目评审" --filter '{"time_range":{"start_time":"2026-03-16T00:00:00+08:00","end_time":"2026-03-22T23:59:59+08:00"}}'

# 指定文件夹
bytedcli lark mail +triage --filter '{"folder":"sent"}'

# 系统标签（可通过 folder 或 label 传入，搜索时自动转为 folder）
bytedcli lark mail +triage --filter '{"folder":"flagged"}'
bytedcli lark mail +triage --filter '{"label":"important"}'
bytedcli lark mail +triage --filter '{"label":"重要邮件"}'

# json/data 格式可配合 jq 处理
bytedcli lark mail +triage --format json | jq '.messages[].subject'

# 分页：先取 10 条，再用 page_token 翻页
bytedcli lark mail +triage --max 10 --format json
# 输出中包含 page_token，传入下一次请求
bytedcli lark mail +triage --page-token 'list:FfccvoqPd...' --max 10 --format json

# --page-size 是 --max 的别名
bytedcli lark mail +triage --page-size 10
```
