...
本 skill 对应 shortcut `bytedcli lark mail +thread`，内部调用：
...
```bash
# 读取完整会话
bytedcli lark mail +thread --thread-id <thread-id>

# 仅纯文本正文（更小的负载，适合 AI 处理）
bytedcli lark mail +thread --thread-id <thread-id> --html=false

# 指定邮箱
bytedcli lark mail +thread --mailbox user@example.com --thread-id <thread-id>

# JSON 输出
bytedcli lark mail +thread --thread-id <thread-id> --format json

# Dry Run
bytedcli lark mail +thread --thread-id <thread-id> --dry-run
```
...
```bash
bytedcli lark mail +thread --thread-id <thread_id> --format json | jq -r '.data.messages[0].body_html'
```
...
```bash
# 1. 从某封邮件获取 thread_id
bytedcli lark mail +message --message-id <id> --html=false --format json | jq '.data.thread_id'
...
# 回复
bytedcli lark mail +reply --message-id <last_message_id> --body "..."
```

## 相关命令

- `bytedcli lark mail +message` — 读取单封邮件
...
- `bytedcli lark mail user_mailbox.messages list` — 列出收件箱邮件（获取 `thread_id`）
