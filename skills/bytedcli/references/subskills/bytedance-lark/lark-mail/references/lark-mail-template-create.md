...
如需修改已有模板，使用 [`bytedcli lark mail +template-update`](./lark-mail-template-update.md)。
...
```bash
# 纯 HTML 模板
bytedcli lark mail +template-create --as user \
  --name '周报模板' \
  --subject '本周进展' \
...
- 原生 API：
  - `bytedcli lark mail user_mailbox.templates list --params '{"user_mailbox_id":"me"}'` — 列出模板
  - `bytedcli lark mail user_mailbox.templates get --params '{"user_mailbox_id":"me","template_id":"<id>"}'` — 获取完整模板
  - `bytedcli lark mail user_mailbox.templates delete --params '{"user_mailbox_id":"me","template_id":"<id>"}'` — 删除
