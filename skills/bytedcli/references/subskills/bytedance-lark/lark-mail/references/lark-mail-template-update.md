...
如需创建新模板，使用 [`bytedcli lark mail +template-create`](./lark-mail-template-create.md)。
...
```bash
# 查看当前状态（不修改）
bytedcli lark mail +template-update --as user --template-id 712345 --inspect

# 打印 patch 骨架并保存
bytedcli lark mail +template-update --as user --print-patch-template > /tmp/tpl-patch.json

# 用扁平 flag 改 subject + cc
bytedcli lark mail +template-update --as user --template-id 712345 \
  --set-subject '每周五发布' \
...
- 删除模板（原生 API）：`bytedcli lark mail user_mailbox.templates delete --params '{"user_mailbox_id":"me","template_id":"<id>"}'`
