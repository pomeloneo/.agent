...
# Dry Run
bytedcli lark mail +share-to-chat --message-id <邮件ID> --receive-id oc_xxx --dry-run
```
...
bytedcli lark im +chat-search --query "项目群"
# → 获取 chat_id: oc_xxx
...
```bash
bytedcli lark mail +share-to-chat --message-id <邮件ID> --receive-id alice@example.com --receive-id-type email
```
...
| 403 `permission not granted` | 缺少 `im:message` 或 `im:message.send_as_user` scope | 重新授权：`bytedcli lark auth login --scope "im:message,im:message.send_as_user"` |

## 相关命令

- `bytedcli lark im +chat-search` — 搜索群聊获取 chat_id
- `bytedcli lark mail +message` — 查看邮件内容
- `bytedcli lark mail +thread` — 查看邮件会话
