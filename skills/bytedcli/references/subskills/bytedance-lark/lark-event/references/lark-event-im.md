...
> **Default = no `--jq`.** Run `bytedcli lark event consume im.message.receive_v1 --as bot` to see every message. The recipes below are only for cases where the user has asked to narrow the stream.
...
Get your own open_id via `bytedcli lark contact +get-user --as user`; other users' via `bytedcli lark contact +search-user`.