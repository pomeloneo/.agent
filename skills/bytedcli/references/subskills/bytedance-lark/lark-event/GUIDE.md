...
name: lark-event
version: 1.0.0
description: "Lark/Feishu real-time event listening / subscribing / consuming: stream events as NDJSON via `bytedcli lark event consume <EventKey>` (covers IM message receive, reactions, chat member changes, etc.). Use for Lark bots, real-time message processing, long-running subscribers, streaming webhook/push handlers. Supports `--max-events` / `--timeout` bounded runs and a stderr ready-marker contract — designed for AI agents running as subprocesses."
---
...
| Command | Purpose |
|------|------|
| `bytedcli lark event list [--json]` | List all subscribable EventKeys |
| `bytedcli lark event schema <EventKey> [--json]` | Show an EventKey's params and output schema |
| `bytedcli lark event consume <EventKey> [flags]` | Blocking consume; events → stdout NDJSON |
| `bytedcli lark event status [--json] [--fail-on-orphan]` | Inspect the local bus daemon status |
| `bytedcli lark event stop [--all] [--force]` | Stop the bus daemon |
...
bytedcli lark event consume im.message.receive_v1 --as bot

# Grab one sample event to inspect payload shape
bytedcli lark event consume im.message.receive_v1 --max-events 1 --timeout 30s --as bot

# Run for 10 minutes then auto-exit
bytedcli lark event consume im.message.receive_v1 --timeout 10m --as bot

# Consume multiple EventKeys concurrently (one shape per process, no dispatcher)
bytedcli lark event consume im.message.receive_v1          --as bot > receive.ndjson &
bytedcli lark event consume im.message.reaction.created_v1 --as bot > reaction.ndjson &
wait
...
1. `bytedcli lark event list --json` → pick a legal key
2. `bytedcli lark event schema <key> --json` → read `resolved_output_schema` + `jq_root_path` to determine field paths
3. `bytedcli lark event consume <key> [--jq '<expr>']` → consume
