...
```bash
# 默认：未指定位置时添加全文评论
bytedcli lark drive +add-comment \
...
bytedcli lark schema drive.file.comments.create_v2

bytedcli lark drive file.comments create_v2 \
...
- 如果需要更底层的控制，仍可改用 `bytedcli lark schema drive.file.comments.create_v2` + `bytedcli lark drive file.comments create_v2`。
