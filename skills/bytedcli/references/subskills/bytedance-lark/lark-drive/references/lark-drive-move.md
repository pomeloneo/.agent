...
```bash
bytedcli lark wiki spaces get --params '{"space_id":"my_library"}'
```
...
- **继续查询**：当看到 `next_command` 时，改用 `bytedcli lark drive +task_result --scenario task_check --task-id <TASK_ID>` 继续查询
