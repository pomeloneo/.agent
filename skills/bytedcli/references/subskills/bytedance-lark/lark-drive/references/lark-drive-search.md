...
> 正确：`bytedcli lark drive +search --query "方案"`
> 错误：`bytedcli lark drive +search 方案`
...
| 用户说 | 命令 |
|---|---|
| 最近一个月我编辑过的文档 | `bytedcli lark drive +search --query "" --edited-since 1m` |
| 最近一个月我编辑过 且 我评论过的 | `bytedcli lark drive +search --query "" --edited-since 1m --commented-since 1m` |
| 最近一周我打开过的表格 | `bytedcli lark drive +search --query "" --opened-since 7d --doc-types sheet` |
| 我创建的所有文档 | `bytedcli lark drive +search --query "" --mine` |
| 我 30-60 天前创建的文档（粗略"上个月"，按 30 天滑窗算） | `bytedcli lark drive +search --query "" --mine --created-since 2m --created-until 1m` |
| 我 2026 年 3 月创建的文档（精确日历月） | `bytedcli lark drive +search --query "" --mine --created-since 2026-03-01 --created-until 2026-04-01` |
| 关键词"预算"，最近一周我打开过，按编辑时间降序 | `bytedcli lark drive +search --query 预算 --opened-since 7d --sort edit_time` |
| 某个 wiki space 下、我 30-60 天前创建的 | `bytedcli lark drive +search --query "" --mine --space-ids space_xxx --created-since 2m --created-until 1m` |
| 张三创建的文档 | `bytedcli lark drive +search --query "" --creator-ids ou_zhangsan` |
| 我最近 3 个月评论过的 docx | `bytedcli lark drive +search --query "" --commented-since 3m --doc-types docx` |

### 更多示例

```bash
# 纯关键词搜索
bytedcli lark drive +search --query "季度总结"

# 使用服务端 query 高级语法（和 docs +search 一致）
bytedcli lark drive +search --query 'intitle:方案'
bytedcli lark drive +search --query '"季度 总结"'
bytedcli lark drive +search --query '方案 OR 草稿'
bytedcli lark drive +search --query '方案 -草稿'

# 只搜某个文件夹下的文档
bytedcli lark drive +search --query 方案 --folder-tokens fld_123456

# 只搜某个知识空间下的 Wiki
bytedcli lark drive +search --query 研发规范 --space-ids space_1234567890fedcba

# 指定群内分享过的文档
bytedcli lark drive +search --query 方案 --chat-ids oc_1234567890abcdef

# 只搜标题 / 只搜评论
bytedcli lark drive +search --query 周报 --only-title
bytedcli lark drive +search --query 延期原因 --only-comment

# 人类可读格式
bytedcli lark drive +search --query OKR --format pretty

# 翻页（--format json 先拿 page_token）
bytedcli lark drive +search --query 方案 --format json
bytedcli lark drive +search --query 方案 --page-token '<PAGE_TOKEN>'
```
...
| 参数 | 映射 | 说明 |
|---|---|---|
| `--mine` | `creator_ids = [当前用户 open_id]` | bool。一键"我创建的"；从当前登录用户身份（`runtime.UserOpenId()`）解析 open_id，取不到直接报错（提示运行 `bytedcli lark auth login`） |
