...
> **导入分流规则：** 如果用户要把本地 Excel / CSV / `.base` 快照导入成 Base / 多维表格 / bitable，必须优先使用 `bytedcli lark drive +import --type bitable`。不要先切到 `lark-base`；`lark-base` 只负责导入完成后的表内操作。

## 快速决策

- 用户要**搜文档 / Wiki / 电子表格 / 多维表格 / 云空间对象**，优先使用 `bytedcli lark drive +search`。自然语言里"最近我编辑过的"、"我创建的"、"最近一周我打开过的 xxx"、"某人创建的 docx" 等直接映射到扁平 flag，避免手写嵌套 JSON。老的 `docs +search` 进入维护期、后续会下线，不要新增对它的依赖。
- 用户要把本地 `.xlsx` / `.csv` / `.base` 导入成 Base / 多维表格 / bitable，第一步必须使用 `bytedcli lark drive +import --type bitable`。
- 用户要把本地 `.md` / `.docx` / `.doc` / `.txt` / `.html` 导入成在线文档，使用 `bytedcli lark drive +import --type docx`。
- 用户要在 Drive 里上传、创建、读取、覆盖更新**原生 `.md` 文件**（不是导入成 docx），切到 [`lark-markdown`](../lark-markdown/GUIDE.md)。
- 用户要把本地 `.xlsx` / `.xls` / `.csv` 导入成电子表格，使用 `bytedcli lark drive +import --type sheet`。
- 用户要在云空间里新建文件夹，优先使用 `bytedcli lark drive +create-folder`。
- 用户要把本地文件上传到知识库 / 文档库里的某个 wiki 节点下时，仍然使用 `bytedcli lark drive +upload --wiki-token <wiki_token>`；不要误切到 `wiki` 域命令。
...
1. **使用 `wiki.spaces.get_node` 查询节点信息**
   ```bash
   bytedcli lark wiki spaces get_node --params '{"token":"wiki_token"}'
   ```
...
- 如果需要更底层地直接调用评论 V2 协议，再走原生 API：先执行 `bytedcli lark schema drive.file.comments.create_v2`，再执行 `bytedcli lark drive file.comments create_v2 ...`。全文评论省略 `anchor`，局部评论传 `anchor.block_id`。
...
```bash
# 默认查询：仅未解决评论（推荐）
bytedcli lark drive file.comments list --params '{"file_token": "xxx", "file_type": "docx", "is_solved": false}'

# 查询所有评论（用户未明确要求包含已解决评论）
bytedcli lark drive file.comments list --params '{"file_token": "xxx", "file_type": "docx", "is_solved": false}'

# 包含已解决评论（需用户明确要求）
bytedcli lark drive file.comments list --params '{"file_token": "xxx", "file_type": "docx"}'
```
...
```bash
# 1. 获取当前应用的 open_id
bytedcli lark api GET /open-apis/bot/v3/info --as bot
# 从返回值中取 bot.open_id

# 2. 授权当前应用访问文档
bytedcli lark drive permission.members create \
...
Shortcut 是对常用操作的高级封装（`bytedcli lark drive +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema drive.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark drive <resource> <method> [flags] # 调用 API
```
