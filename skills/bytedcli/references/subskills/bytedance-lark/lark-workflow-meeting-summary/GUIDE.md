...
```bash
bytedcli lark auth login --domain vc        # 基础（查询+纪要）
bytedcli lark auth login --domain vc,drive   # 含读取纪要文档正文、生成文档
```
...
```bash
# page-size 最大为 30
bytedcli lark vc +search --start "<YYYY-MM-DD>" --end "<YYYY-MM-DD>" --format json --page-size 30
```
...
1. 查询会议关联的纪要信息
```bash
bytedcli lark vc +notes --meeting-ids "id1,id2,...,idN"   
```
...
```bash
bytedcli lark docs +create --api-version v2 --doc-format markdown --content $'<title>会议纪要汇总 (<start> - <end>)</title>\n<内容>'
# 或追加到已有文档
bytedcli lark docs +update --api-version v2 --doc "<url_or_token>" --command append --doc-format markdown --content $'<内容>'
```
