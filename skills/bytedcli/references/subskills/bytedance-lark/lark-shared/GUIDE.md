...
name: lark-shared
version: 1.0.0
description: "飞书/Lark CLI 共享基础：应用配置初始化、认证登录（auth login）、身份切换（--as user/bot）、权限与 scope 管理、Permission denied 错误处理、安全规则。当用户需要第一次配置(`bytedcli lark config init`)、使用登录授权(`bytedcli lark auth login`)、遇到权限不足、切换 user/bot 身份、配置 scope、或首次使用 bytedcli lark 时触发。"
---
...
首次使用需运行 `bytedcli lark config init` 完成应用配置。

当你帮用户初始化配置时，使用background方式使用下面的命令发起配置应用流程，启动后读取输出，从中提取授权链接并发给用户：

```bash
# 发起配置（该命令会阻塞直到用户打开链接并完成操作或过期）
bytedcli lark config init --new
```
...
| 身份 | 标识 | 获取方式 | 适用场景 |
|------|------|---------|---------|
| user 用户身份 | `--as user` | `bytedcli lark auth login` 等 | 访问用户自己的资源（日历、云空间等） |
...
```bash
bytedcli lark auth login --domain <domain>           # 按业务域授权
bytedcli lark auth login --scope "<missing_scope>"   # 按具体 scope 授权（推荐,符合最小权限原则）
```
...
```bash
# 发起授权（阻塞直到用户授权完成或过期）
bytedcli lark auth login --scope "calendar:calendar:readonly"
...
bytedcli lark 命令执行后，如果检测到新版本，JSON 输出中会包含 `_notice.update` 字段（含 `message`、`command` 等）。
...
bytedcli lark 对高风险写操作（`risk: "high-risk-write"`）有强制确认门禁。当你不带 `--yes` 调用这类命令时，CLI 会退出码 `10`、并在 stderr 返回如下结构化 envelope：
...
- shortcut：`bytedcli lark <service> +<cmd> --help` 顶部会显示 `Risk: high-risk-write`
- service 命令：`bytedcli lark schema <service>.<resource>.<method> --format json` 的返回值里 `"risk": "high-risk-write"`
