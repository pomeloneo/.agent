...
```bash
bytedcli lark base +record-batch-update --base-token <base_token> --table-id <table_id> \
  --json '{"record_id_list":["<record_id>"],"patch":{"状态":"完成"}}'

bytedcli lark base +record-batch-update --base-token <base_token> --table-id <table_id> --json @batch-update.json
```
