...
```bash
# 创建记录
bytedcli lark base +record-upsert --base-token <base_token> --table-id <table_id> \
  --json '{"项目名称":"Apollo","状态":"进行中"}'

# 更新记录
bytedcli lark base +record-upsert --base-token <base_token> --table-id <table_id> --record-id <record_id> \
