...
2. **先查表结构**：执行 `bytedcli lark base +field-list --base-token <base_token> --table-id <table_id>`
...
- ⚠️ **必须先查表结构**：DSL 的 `field_name` 必须与表中字段名称精确匹配（区分大小写），不能凭猜测构造。先用 `bytedcli lark base +field-list --base-token <base_token> --table-id <table_id>` 获取真实字段名
