...
```bash
bytedcli lark base +record-upload-attachment \
...
- ⚠️ 记录里的附件 `file_token` 属于 Drive media token；下载时不要走 `bytedcli lark drive +download`，应使用 `bytedcli lark docs +media-download --token <file_token> --output <path>`。
