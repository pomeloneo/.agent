...
> 适用命令：`bytedcli lark base +record-upsert`、`bytedcli lark base +record-batch-create`、`bytedcli lark base +record-batch-update`
...
> **人员字段：不要猜 ID。** 不知道 `open_id` 时，先用 `lark-contact` 查 id：`bytedcli lark contact +search-user --query "<姓名/邮箱/手机号>" --as user`。

> **群组字段：不要猜 ID。** 不知道 `chat_id` 时，先用 `lark-im` 搜群：`bytedcli lark im +chat-search --query "<群名关键词>" --as user`；取结果里的 `oc_xxx`。
...
用户要把本地文件加到记录里时，必须使用 `bytedcli lark base +record-upload-attachment --file <path>` 上传到已有记录。不能用普通记录操作接口来上传附件。

`+record-get` 返回的附件字段单元格包含 `file_token` 和文件名，可以把 `file_token` 交给 `bytedcli lark docs +media-download` 进行附件下载。
