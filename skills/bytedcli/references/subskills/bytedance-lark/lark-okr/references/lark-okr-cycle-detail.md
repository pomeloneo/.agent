...
```bash
# 列出指定周期的目标和关键结果
bytedcli lark okr +cycle-detail --cycle-id 1234567890123456789

# 预览 API 调用而不实际执行
bytedcli lark okr +cycle-detail --cycle-id 1234567890123456789 --dry-run
```
...
1. 使用 `bytedcli lark okr +cycle-list` 获取 OKR 周期 ID。
2. 执行 `bytedcli lark okr +cycle-detail --cycle-id "123456"`。
