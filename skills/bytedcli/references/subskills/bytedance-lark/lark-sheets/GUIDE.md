...
## 快速决策
- 按标题或关键词找云空间里的表格文件，先用 `bytedcli lark docs +search`。
...
```bash
# ✅ 正确：使用对象格式
bytedcli lark sheets +write --url "URL" --sheet-id "sheetId" --range "C6" \
...
Shortcut 是对常用操作的高级封装（`bytedcli lark sheets +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
> 浮动图片相关的读接口只返回元数据（含 `float_image_token`），**不包含图片字节**。要读取图片内容，用 token 调 `bytedcli lark docs +media-preview --token "<float_image_token>" --output ./image.png`。

## API Resources

```bash
bytedcli lark schema sheets.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark sheets <resource> <method> [flags] # 调用 API
```
