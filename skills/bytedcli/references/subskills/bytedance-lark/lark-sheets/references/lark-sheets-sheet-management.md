...
对应命令：`bytedcli lark sheets +create-sheet`

```bash
# 在表格末尾或服务端默认位置创建工作表
bytedcli lark sheets +create-sheet --spreadsheet-token "shtxxxxxxxx" \
  --title "明细"

# 指定插入位置（0-based）
bytedcli lark sheets +create-sheet --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --title "汇总" --index 0
```
...
对应命令：`bytedcli lark sheets +copy-sheet`

```bash
# 按默认位置复制
bytedcli lark sheets +copy-sheet --spreadsheet-token "shtxxxxxxxx" \
  --sheet-id "<sheetId>"

# 指定副本名称和位置
bytedcli lark sheets +copy-sheet --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +delete-sheet`

> [!CAUTION]
> 这是**高风险删除操作**。CLI 会要求显式确认；可以先用 `--dry-run` 预览。

```bash
bytedcli lark sheets +delete-sheet --spreadsheet-token "shtxxxxxxxx" \
  --sheet-id "<sheetId>"
```
...
对应命令：`bytedcli lark sheets +update-sheet`

用于更新工作表标题、位置、隐藏状态、冻结行列和保护设置。

```bash
# 改名 + 调整冻结
bytedcli lark sheets +update-sheet --spreadsheet-token "shtxxxxxxxx" \
  --sheet-id "<sheetId>" --title "汇总表" --frozen-row-count 2 --frozen-col-count 1

# 隐藏工作表
bytedcli lark sheets +update-sheet --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
