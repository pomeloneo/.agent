...
对应命令：`bytedcli lark sheets +read`
...
```bash
bytedcli lark sheets +read --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --range "<sheetId>!A1:H20"

bytedcli lark sheets +read --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +write`

用于覆盖写入一个矩形区域。

```bash
bytedcli lark sheets +write --spreadsheet-token "shtxxxxxxxx" \
  --range "<sheetId>!A1:B2" \
  --values '[["name","age"],["alice",18]]'

bytedcli lark sheets +write --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --sheet-id "<sheetId>" --range "C2" \
  --values '[["hello"]]'
```
...
对应命令：`bytedcli lark sheets +append`

用于向工作表末尾追加行。

```bash
bytedcli lark sheets +append --spreadsheet-token "shtxxxxxxxx" \
  --range "<sheetId>!A1" \
...
对应命令：`bytedcli lark sheets +find`

只在一个已知 spreadsheet 内查找单元格内容，不是云空间搜索。

```bash
bytedcli lark sheets +find --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --sheet-id "<sheetId>" --find "张三" --range "A1:H200"

bytedcli lark sheets +find --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +replace`

在指定范围内查找并替换单元格内容。

```bash
bytedcli lark sheets +replace --spreadsheet-token "shtxxxxxxxx" \
