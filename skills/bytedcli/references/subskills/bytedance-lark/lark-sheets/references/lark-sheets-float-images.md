...
对应命令：`bytedcli lark sheets +media-upload`

把本地图片上传到指定电子表格的素材空间，返回 `file_token`，供 `+create-float-image` 使用。

```bash
bytedcli lark sheets +media-upload --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --file ./image.png
```
...
对应命令：`bytedcli lark sheets +create-float-image`

```bash
bytedcli lark sheets +create-float-image --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +update-float-image`

```bash
bytedcli lark sheets +update-float-image --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +get-float-image`

```bash
bytedcli lark sheets +get-float-image --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --sheet-id "<sheetId>" --float-image-id "fi12345678"
```
...
对应命令：`bytedcli lark sheets +list-float-images`

```bash
bytedcli lark sheets +list-float-images --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --sheet-id "<sheetId>"
```
...
对应命令：`bytedcli lark sheets +delete-float-image`

```bash
bytedcli lark sheets +delete-float-image --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --sheet-id "<sheetId>" --float-image-id "fi12345678"
```
...
```bash
bytedcli lark docs +media-preview --token "<float_image_token>" --output ./image.png
```
