...
对应命令：`bytedcli lark sheets +create`
...
```bash
# 只创建表格
bytedcli lark sheets +create --title "仓库管理营收报表"
...
# 创建到指定文件夹
bytedcli lark sheets +create --title "测试表" --folder-token "fldbc_xxx"

# 仅预览请求
bytedcli lark sheets +create --title "测试表" --dry-run
```
...
对应命令：`bytedcli lark sheets +info`
...
```bash
# 传 URL（支持 wiki URL）
bytedcli lark sheets +info --url "https://example.larksuite.com/sheets/shtxxxxxxxx"

# 传 spreadsheet_token
bytedcli lark sheets +info --spreadsheet-token "shtxxxxxxxx"

# 仅预览请求
bytedcli lark sheets +info --url "https://..." --dry-run
```
...
对应命令：`bytedcli lark sheets +export`
...
```bash
# 导出 xlsx 并保存到本地
bytedcli lark sheets +export --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
  --file-extension xlsx --output-path "./report.xlsx"

# 导出 csv（必须指定 sheet-id）
bytedcli lark sheets +export --spreadsheet-token "shtxxxxxxxx" \
  --file-extension csv --sheet-id "<sheetId>" --output-path "./report.csv"

# 只返回导出文件 token
bytedcli lark sheets +export --spreadsheet-token "shtxxxxxxxx" --file-extension xlsx
```
