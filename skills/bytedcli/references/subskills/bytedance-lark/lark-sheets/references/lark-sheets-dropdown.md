...
对应命令：`bytedcli lark sheets +set-dropdown`

```bash
bytedcli lark sheets +set-dropdown --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +update-dropdown`

```bash
bytedcli lark sheets +update-dropdown --spreadsheet-token "shtxxxxxxxx" \
  --sheet-id "<sheetId>" \
...
对应命令：`bytedcli lark sheets +get-dropdown`

```bash
bytedcli lark sheets +get-dropdown --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +delete-dropdown`

```bash
bytedcli lark sheets +delete-dropdown --spreadsheet-token "shtxxxxxxxx" \
...
```bash
# 1. 配置下拉
bytedcli lark sheets +set-dropdown --url "<url>" \
  --range "<sheetId>!J2:J100" --condition-values '["选项1","选项2"]' --multiple

# 2. 再写入 multipleValue
bytedcli lark sheets +write --url "<url>" --sheet-id "<sheetId>" --range "J2" \
