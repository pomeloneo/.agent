...
对应命令：`bytedcli lark sheets +set-style`

对指定范围设置字体、颜色、对齐、边框等样式。

```bash
bytedcli lark sheets +set-style --spreadsheet-token "shtxxxxxxxx" \
  --range "<sheetId>!A1:C3" \
...
对应命令：`bytedcli lark sheets +batch-set-style`

对多个范围批量设置不同样式。

```bash
bytedcli lark sheets +batch-set-style --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +merge-cells`
...
```bash
bytedcli lark sheets +merge-cells --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +unmerge-cells`

用于拆分合并单元格。

```bash
bytedcli lark sheets +unmerge-cells --spreadsheet-token "shtxxxxxxxx" \
  --range "<sheetId>!A1:B2"
```
