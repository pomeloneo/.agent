...
对应命令：`bytedcli lark sheets +write-image`
...
```bash
# 写入图片到指定单元格
bytedcli lark sheets +write-image --spreadsheet-token "shtxxxxxxxx" \
  --range "<sheetId>!B2:B2" \
  --image "./logo.png"

# 使用 URL + sheet-id，指定单个单元格
bytedcli lark sheets +write-image --url "https://example.larksuite.com/sheets/shtxxxxxxxx" \
