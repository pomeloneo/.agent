...
对应命令：`bytedcli lark sheets +add-dimension`

在工作表末尾追加空行或空列，不影响已有数据。

```bash
bytedcli lark sheets +add-dimension --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +insert-dimension`

在指定位置插入空行或空列，已有数据向下或向右移动。

```bash
bytedcli lark sheets +insert-dimension --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +update-dimension`

更新指定范围行/列的显隐状态和行高/列宽。

```bash
bytedcli lark sheets +update-dimension --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +move-dimension`

将指定范围的行/列移动到目标位置。

```bash
bytedcli lark sheets +move-dimension --spreadsheet-token "shtxxxxxxxx" \
...
对应命令：`bytedcli lark sheets +delete-dimension`

删除指定范围的行或列。

```bash
bytedcli lark sheets +delete-dimension --spreadsheet-token "shtxxxxxxxx" \
  --sheet-id "<sheetId>" --dimension ROWS --start-index 3 --end-index 7
```
