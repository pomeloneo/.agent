...
本 skill 对应 shortcut：`bytedcli lark minutes +upload`。
...
1. **上传文件至云空间获取 file_token**
   - 使用 `bytedcli lark drive +upload` 命令上传本地文件到云空间（Drive）：
     ```bash
     bytedcli lark drive +upload --file <path/to/media/file>
     ```
...
2. **将 file_token 转换为妙记链接（minute_url）**
   - 调用本 shortcut，将获取到的 `file_token` 转换为妙记：
     ```bash
     bytedcli lark minutes +upload --file-token <file_token>
     ```
...
bytedcli lark minutes +upload --file-token boxcnxxxxxxxxxxxxxxxx

# 通过 minute_token 继续获取纪要 / 逐字稿 / 文字稿 / AI 产物
bytedcli lark vc +notes --minute-tokens obcnxxxxxxxxxxxxxxxx
```
...
1. 使用 `bytedcli lark drive +upload --file <path>` 上传本地音视频文件到云空间
2. 从返回结果中取出 `file_token`
3. 调用 `bytedcli lark minutes +upload --file-token <file_token>` 生成妙记
4. 如果目标是纪要、逐字稿、文字稿、撰写文字、总结、待办或章节，再从 `minute_url` 提取 `minute_token`，继续调用 `bytedcli lark vc +notes --minute-tokens <minute_token>`
