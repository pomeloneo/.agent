...
> 使用 `bytedcli lark schema minutes.minutes.get` 可查看完整返回值结构。核心字段包含：`title`（标题）、`cover`（封面 URL）、`duration`（时长，毫秒）、`owner_id`（所有者 ID）、`url`（妙记链接）。
...
3. **处理流程**：
   - **上传音视频获取 `file_token`**：使用 [`bytedcli lark drive +upload`](../lark-drive/references/lark-drive-upload.md) 上传本地文件到云空间并获取 `file_token`。
   - **生成妙记**：获取到 `file_token` 后，调用 [`bytedcli lark minutes +upload`](references/lark-minutes-upload.md) 将文件转换为妙记并获取 `minute_url` 链接。
   - **继续获取纪要 / 逐字稿（按需）**：如果用户目标不是只要妙记链接，而是要纪要、逐字稿、总结、待办或章节，则从 `minute_url` 中提取 `minute_token`，再调用 [`bytedcli lark vc +notes --minute-tokens`](../lark-vc/references/lark-vc-notes.md) 获取对应产物。
...
Shortcut 是对常用操作的高级封装（`bytedcli lark minutes +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema minutes.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark minutes <resource> <method> [flags] # 调用 API
```
