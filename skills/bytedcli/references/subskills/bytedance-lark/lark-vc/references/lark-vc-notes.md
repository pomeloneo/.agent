...
本 skill 对应 shortcut：`bytedcli lark vc +notes`。

## 命令

```bash
# 通过会议 ID 查询（逗号分隔支持批量，最多 50 个）
bytedcli lark vc +notes --meeting-ids 69xxxxxxxxxxxxx28
bytedcli lark vc +notes --meeting-ids 69xxxxxxxxxxxxx28,69xxxxxxxxxxxxx29

# 通过妙记 Token 查询（从妙记 URL 中提取）
bytedcli lark vc +notes --minute-tokens obbxxxxxxxxxxxxxxxxxx
bytedcli lark vc +notes --minute-tokens obbxxxxxxxxxxxxxxxxxx,obbyyyyyyyyyyyyyyyyyy

# 指定逐字稿输出目录（仅 --minute-tokens 路径有效）
bytedcli lark vc +notes --minute-tokens obbxxxxxxxxxxxxxxxxxx --output-dir ./output
bytedcli lark vc +notes --minute-tokens obbxxxxxxxxxxxxxxxxxx --overwrite

# 通过日程事件 ID 查询（从 calendar +agenda 获取 event_id）
bytedcli lark vc +notes --calendar-event-ids xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_0

# 输出格式
bytedcli lark vc +notes --meeting-ids 69xxxxxxxxxxxxx28 --format json

# 预览 API 调用
bytedcli lark vc +notes --meeting-ids 69xxxxxxxxxxxxx28 --dry-run
```
...
该命令仅支持 `user` 身份，使用前需完成 `bytedcli lark auth login`。
