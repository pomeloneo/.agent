...
本 skill 对应 shortcut：`bytedcli lark vc +search`（调用 `POST /open-apis/vc/v1/meetings/search`）。
...
```bash
# 关键词搜索
bytedcli lark vc +search --query "周会"

# 查询某一天开过的会（单日查询时，start 和 end 必须填写同一天）
bytedcli lark vc +search --start 2026-03-10 --end 2026-03-10

# 按时间范围搜索
bytedcli lark vc +search --start "2026-03-10T00:00+08:00" --end "2026-03-17T00:00+08:00"
bytedcli lark vc +search --start 2026-03-10 --end 2026-03-17

# 关键词 + 时间范围
bytedcli lark vc +search --query "周会" --start "2026-03-10T00:00+08:00" --end "2026-03-17T00:00+08:00"
bytedcli lark vc +search --query "周会" --start "2026-03-10T00:00+08:00"
bytedcli lark vc +search --query "周会" --end "2026-03-17T00:00+08:00"

# 按组织者过滤（open_id，逗号分隔）
bytedcli lark vc +search --organizer-ids "ou_a,ou_b"

# 按参与者过滤（open_id，逗号分隔）
bytedcli lark vc +search --participant-ids "ou_x,ou_y"

# 按会议室过滤
bytedcli lark vc +search --room-ids "123,456"

# 多条件组合查询
bytedcli lark vc +search --organizer-ids "ou_a" --room-ids "123" --start "2026-03-10T00:00+08:00"

# 分页查询
bytedcli lark vc +search --query "周会" --page-size 15
bytedcli lark vc +search --query "周会" --page-token "next_page_token"

# 输出为表格/可读格式
bytedcli lark vc +search --query "周会" --format json
```
...
该接口仅支持 `user` 身份，使用前需完成 `bytedcli lark auth login` 并具备 `vc:meeting.search:read` 权限。
...
# Next page
bytedcli lark vc +search --query "周会" --page-size 15 --page-token "<PAGE_TOKEN>"
```
...
bytedcli lark vc +notes --meeting-ids <MEETING_ID>

# 如果要会议对应的妙记信息 / minute_token / 妙记链接
bytedcli lark vc +recording --meeting-ids <MEETING_ID>
# 然后再用返回的 minute_token 调用：
bytedcli lark minutes minutes get --params '{"minute_token":"<MINUTE_TOKEN>"}'
```
