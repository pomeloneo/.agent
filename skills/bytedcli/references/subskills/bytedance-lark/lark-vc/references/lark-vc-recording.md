...
本 skill 对应 shortcut：`bytedcli lark vc +recording`。

## 命令

```bash
# 通过会议 ID 查询（逗号分隔支持批量，最多 50 个）
bytedcli lark vc +recording --meeting-ids 69xxxxxxxxxxxxx28
bytedcli lark vc +recording --meeting-ids 69xxxxxxxxxxxxx28,69xxxxxxxxxxxxx29

# 通过日程事件 ID 查询
bytedcli lark vc +recording --calendar-event-ids xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_0

# 输出格式
bytedcli lark vc +recording --meeting-ids 69xxxxxxxxxxxxx28 --format json

# 预览 API 调用
bytedcli lark vc +recording --meeting-ids 69xxxxxxxxxxxxx28 --dry-run
```
...
该命令仅支持 `user` 身份，使用前需完成 `bytedcli lark auth login`。user token 只能查自己有权限的录制。
...
| 输入参数 | 获取方式 |
|---------|---------|
| `meeting_id` | 使用 `bytedcli lark vc +search` 搜索历史会议，取结果中的 `id` 字段 |
| `calendar_event_id` | 使用 `bytedcli lark calendar +agenda` 查看日程，取结果中的 `event_id` 字段 |
...
bytedcli lark vc +recording --meeting-ids xxx

# 第 2 步：使用上一步返回的 minute_token 下载妙记文件
bytedcli lark minutes +download --minute-token <minute_token>
```
...
bytedcli lark vc +notes --minute-tokens <minute_token>
```
...
bytedcli lark vc +search --query "周会" --start yesterday

# 第 2 步：使用上一步返回的 meeting_ids 查询录制，拿到 minute_tokens
bytedcli lark vc +recording --meeting-ids <ids>

# 第 3 步：使用其中一个 minute_token 下载妙记文件
bytedcli lark minutes +download --minute-token <token>
```
...
bytedcli lark vc +recording --calendar-event-ids <event_id>
