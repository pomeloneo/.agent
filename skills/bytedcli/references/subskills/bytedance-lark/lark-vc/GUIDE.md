...
```bash
# 1. 读取纪要内容
bytedcli lark docs +fetch --api-version v2 --doc <note_doc_token> --doc-format markdown
# 2. 从返回的 markdown 中提取第一个 <whiteboard token="xxx"/> 的 token
# 3. 下载封面图到聚合目录（和逐字稿、录像同目录，保持产物归拢）
#    并非所有纪要都有封面画板，没有 <whiteboard> 标签时跳过即可
bytedcli lark docs +media-download --type whiteboard --token <whiteboard_token> --output ./minutes/<minute_token>/cover
```
...
2. 仅需要获取文档名称和 URL 等基本信息时，使用 `bytedcli lark drive metas batch_query` 查询
```bash
# 学习命令使用方式
bytedcli lark schema drive.metas.batch_query

# 批量获取文档基本信息: 一次最多查询 10 个文档
bytedcli lark drive metas batch_query --data '{"request_docs": [{"doc_type": "docx", "doc_token": "<doc_token>"}], "with_url": true}'
```
3. 需要获取文档内容时，使用 `bytedcli lark docs +fetch`。
```bash
# 获取文档内容
bytedcli lark docs +fetch --api-version v2 --doc <doc_token> --doc-format markdown
```
...
>
> **妙记边界**：`+notes` 负责纪要内容、逐字稿和 AI 产物；妙记基础信息请优先看 [`+recording`](references/lark-vc-recording.md) 与 [lark-minutes](../lark-minutes/GUIDE.md)。
>
> **文件转纪要边界**：如果用户给的是本地音视频文件，并希望得到纪要、逐字稿、总结、待办或章节，入口应先走 [lark-minutes](../lark-minutes/GUIDE.md) 的上传流程生成 `minute_url` / `minute_token`，再回到 `vc +notes --minute-tokens` 获取内容产物。
>
> **特殊情况**: 当用户查询“今天有哪些会议”时，通过 `vc +search` 查询今天开过的会议记录，同时使用 lark-calendar 技能查询今天还未开始的会议，统一整理后展示给用户。

## Shortcuts（推荐优先使用）

Shortcut 是对常用操作的高级封装（`bytedcli lark vc +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema vc.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark vc <resource> <method> [flags] # 调用 API
```
...
```bash
# 获取会议基础信息：不包含参会人列表
bytedcli lark vc meeting get --params '{"meeting_id": "<meeting_id>"}'


# 获取会议基础信息：包含参会人列表
bytedcli lark vc meeting get --params '{"meeting_id": "<meeting_id>", "with_participants": true}'
```
