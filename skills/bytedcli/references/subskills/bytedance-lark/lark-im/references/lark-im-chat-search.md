...
This skill maps to the shortcut: `bytedcli lark im +chat-search` (internally calls `POST /open-apis/im/v2/chats/search`).

## Commands

```bash
# Search chats by keyword
bytedcli lark im +chat-search --query "project"

# Restrict by search types
bytedcli lark im +chat-search --query "project" --search-types "private,public_joined"

# Filter by member open_ids (with keyword)
bytedcli lark im +chat-search --query "project" --member-ids "ou_xxx,ou_yyy"

# Search by member open_ids only
bytedcli lark im +chat-search --member-ids "ou_xxx,ou_yyy"

# Only show chats you created or manage
bytedcli lark im +chat-search --query "project" --is-manager

# Set page size
bytedcli lark im +chat-search --query "project" --page-size 10

# Pagination
bytedcli lark im +chat-search --query "project" --page-token "xxx"

# JSON output
bytedcli lark im +chat-search --query "project" --format json

# Preview the request without executing it
bytedcli lark im +chat-search --query "project" --dry-run
```
...
```bash
bytedcli lark im +chat-search --query "design review"
```

### Scenario 2: Search a chat and list recent messages

```bash
CHAT_ID=$(bytedcli lark im +chat-search --query "project" --format json | jq -r '.data.chats[0].chat_id')
...
```bash
CHAT_ID=$(bytedcli lark im +chat-search --query "daily report" --format json | jq -r '.data.chats[0].chat_id')
bytedcli lark im +messages-send --chat-id "$CHAT_ID" --text "Today's progress update"
```
...
| Permission denied (99991679) with `--as user` | UAT is not authorized for `im:chat:read` | Run `bytedcli lark auth login --scope "im:chat:read"` |
