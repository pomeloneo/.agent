...
- **Owner-only actions such as owner transfer:** require the actual owner to complete UAT authorization via `bytedcli lark auth login`, then perform the action as that owner.
...
1. Create the group with the bot first, excluding invisible users: `bytedcli lark im +chat-create --name "Group Name"`
