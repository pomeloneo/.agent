...
This skill maps to the shortcut: `bytedcli lark im +messages-send` (internally calls `POST /open-apis/im/v1/messages`).
...
```bash
bytedcli lark im +messages-send --chat-id oc_xxx --text $'Build failed\nBranch: feature/im-docs\nAction: please check logs'
```

```bash
bytedcli lark im +messages-send --chat-id oc_xxx --text $'```bash\nmake test\nmake lint\n```'
```
...
```bash
bytedcli lark im +messages-send --chat-id oc_xxx --markdown $'## Release Notes\n\n- Added send shortcut\n- Added reply shortcut'
```
...
bytedcli lark im +messages-send --chat-id oc_xxx --text "Hello"

# Equivalent manual JSON
bytedcli lark im +messages-send --chat-id oc_xxx --content '{"text":"Hello"}'

# Send to a direct message (pass open_id)
bytedcli lark im +messages-send --user-id ou_xxx --text "Hello"

# Send multi-line text while preserving formatting
bytedcli lark im +messages-send --chat-id oc_xxx --text $'Line 1\nLine 2\n  indented line'

# Send basic Markdown (will be converted to post JSON)
bytedcli lark im +messages-send --chat-id oc_xxx --markdown $'## Update\n\n- item 1\n- item 2'

# If you need exact post structure, send JSON directly
bytedcli lark im +messages-send --chat-id oc_xxx --msg-type post --content '{"zh_cn":{"title":"Title","content":[[{"tag":"text","text":"Body"}]]}}'

# Send a local image (uploaded automatically before sending)
bytedcli lark im +messages-send --chat-id oc_xxx --image ./photo.png

# Or send directly with an existing image_key
bytedcli lark im +messages-send --chat-id oc_xxx --image img_xxx

# Send a local file (uploaded automatically before sending)
bytedcli lark im +messages-send --chat-id oc_xxx --file ./report.pdf

# Send a video (--video-cover is required as the cover)
bytedcli lark im +messages-send --chat-id oc_xxx --video ./demo.mp4 --video-cover ./cover.png
bytedcli lark im +messages-send --chat-id oc_xxx --video ./demo.mp4 --video-cover img_xxx

# Send audio
bytedcli lark im +messages-send --chat-id oc_xxx --audio ./voice.opus

# Use an idempotency key (same key sends only once within 1 hour)
bytedcli lark im +messages-send --chat-id oc_xxx --text "Hello" --idempotency-key my-unique-id

# Preview the request without executing it
bytedcli lark im +messages-send --chat-id oc_xxx --markdown $'## Test\n\nhello' --dry-run
```
