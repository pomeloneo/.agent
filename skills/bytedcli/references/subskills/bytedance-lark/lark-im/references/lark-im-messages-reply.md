...
This skill maps to the shortcut: `bytedcli lark im +messages-reply` (internally calls `POST /open-apis/im/v1/messages/:message_id/reply`).
...
```bash
bytedcli lark im +messages-reply --message-id om_xxx --text $'Received\nI will check this today.\nOwner: alice'
```

```bash
bytedcli lark im +messages-reply --message-id om_xxx --text $'```sql\nselect * from jobs;\n```'
```
...
```bash
bytedcli lark im +messages-reply --message-id om_xxx --markdown $'## Follow-up\n\n- I reproduced it\n- I am fixing it'
```
...
bytedcli lark im +messages-reply --message-id om_xxx --content '{"text":"Received"}'

# Reply as a bot
bytedcli lark im +messages-reply --message-id om_xxx --text "bot reply" --as bot

# Reply with preserved multi-line text
bytedcli lark im +messages-reply --message-id om_xxx --text $'Line 1\nLine 2\n  indented line'

# Reply inside the thread (message appears in the target thread)
bytedcli lark im +messages-reply --message-id om_xxx --text "Let's discuss this" --reply-in-thread

# Reply with basic Markdown (will be converted to post JSON)
bytedcli lark im +messages-reply --message-id om_xxx --markdown $'## Reply\n\n- item 1\n- item 2'

# If you need exact post structure, send JSON directly
bytedcli lark im +messages-reply --message-id om_xxx --msg-type post --content '{"zh_cn":{"title":"Reply","content":[[{"tag":"text","text":"Detailed content"}]]}}'

# Reply with a local image (uploaded automatically before sending)
bytedcli lark im +messages-reply --message-id om_xxx --image ./photo.png

# Reply with a local file (uploaded automatically before sending)
bytedcli lark im +messages-reply --message-id om_xxx --file ./report.pdf

# Reply with a local video (--video-cover is required as the video cover)
bytedcli lark im +messages-reply --message-id om_xxx --video ./demo.mp4 --video-cover ./cover.png

# With an idempotency key
bytedcli lark im +messages-reply --message-id om_xxx --text "Received" --idempotency-key my-unique-id

# Preview the request without executing it
bytedcli lark im +messages-reply --message-id om_xxx --markdown $'## Test\n\nhello' --dry-run
```
...
```bash
bytedcli lark im +messages-reply --message-id om_xxx --text "OK, I will handle it"
```
...
```bash
bytedcli lark im +messages-reply --message-id om_xxx --text "Let me take a look at this" --reply-in-thread
```
