...
This skill maps to the shortcut: `bytedcli lark im +chat-messages-list` (internally calls `GET /open-apis/im/v1/messages`, and automatically resolves the p2p chat_id when needed).

## Commands

```bash
# Get group chat messages (json output by default)
bytedcli lark im +chat-messages-list --chat-id oc_xxx

# Get direct messages with a user (pass open_id and resolve p2p chat_id automatically)
bytedcli lark im +chat-messages-list --user-id ou_xxx

# Specify a time range (ISO 8601)
bytedcli lark im +chat-messages-list --chat-id oc_xxx --start "2026-03-10T00:00:00+08:00" --end "2026-03-11T00:00:00+08:00"

# Specify a time range (date only)
bytedcli lark im +chat-messages-list --chat-id oc_xxx --start 2026-03-10 --end 2026-03-11

# Control sort order and page size (max 50)
bytedcli lark im +chat-messages-list --chat-id oc_xxx --sort asc --page-size 20

# Pagination
bytedcli lark im +chat-messages-list --chat-id oc_xxx --page-token "xxx"

# JSON output
bytedcli lark im +chat-messages-list --chat-id oc_xxx --format json
```
...
```bash
bytedcli lark im +threads-messages-list --thread omt_xxx
```
...
```bash
bytedcli lark im +chat-messages-list --chat-id oc_xxx --page-token <PAGE_TOKEN>
```

You can also fall back to the generic API:

```bash
bytedcli lark api GET /open-apis/im/v1/messages \
