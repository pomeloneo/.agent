...
This skill maps to the shortcut: `bytedcli lark im +messages-search` (internally calls `POST /open-apis/im/v1/messages/search` + batched `GET /open-apis/im/v1/messages/mget`, then batch-fetches chat context).

## Commands

```bash
# Search by keyword
bytedcli lark im +messages-search --query "project progress"

# Restrict search to a specific group chat
bytedcli lark im +messages-search --query "weekly report" --chat-id oc_xxx

# Filter by sender (comma-separated)
bytedcli lark im +messages-search --query "requirement" --sender ou_xxx,ou_yyy

# Filter by attachment type
bytedcli lark im +messages-search --query "report" --include-attachment-type file

# Filter by chat type (group / p2p)
bytedcli lark im +messages-search --query "progress" --chat-type group

# Filter by sender type (user / bot)
bytedcli lark im +messages-search --query "reminder" --sender-type bot

# Exclude bot senders
bytedcli lark im +messages-search --query "reminder" --exclude-sender-type bot

# Only messages that @me
bytedcli lark im +messages-search --query "announcement" --is-at-me

# Only messages that @mention specific users (results also include messages that @all)
bytedcli lark im +messages-search --query "release" --at-chatter-ids ou_xxx,ou_yyy

# Combined filters + time range
bytedcli lark im +messages-search --query "meeting" --sender ou_xxx --chat-type group --start "2026-03-13T00:00:00+08:00" --end "2026-03-20T23:59:59+08:00"

# Specific time range (ISO 8601)
bytedcli lark im +messages-search --query "release" --start "2026-03-01T00:00:00+08:00" --end "2026-03-10T00:00:00+08:00"

# Output format options
bytedcli lark im +messages-search --query "test" --format pretty
bytedcli lark im +messages-search --query "test" --format table
bytedcli lark im +messages-search --query "test" --format csv

# Pagination
bytedcli lark im +messages-search --query "test" --page-token <PAGE_TOKEN>

# Auto-pagination across multiple pages
bytedcli lark im +messages-search --query "test" --page-all --format json

# Auto-pagination with an explicit page cap
bytedcli lark im +messages-search --query "test" --page-limit 5 --format json

# Preview the request without executing it
bytedcli lark im +messages-search --query "test" --dry-run
```
...
bytedcli lark im +chat-messages-list --chat-id <chat_id>

# View replies in the thread that contains the search result
bytedcli lark im +threads-messages-list --thread <thread_id>
```
...
bytedcli lark im +chat-search --query "<chat name keyword>" --format json

# Step 2: Use the chat_id to narrow down message search
bytedcli lark im +messages-search --query "keyword" --chat-id <chat_id>
```
...
bytedcli lark im +messages-search --query "" --chat-id oc_xxx --sender ou_me --start "2026-03-18T00:00:00+08:00" --end "2026-03-25T23:59:59+08:00" --page-size 50 --page-all --format json

# If you need to cap the run explicitly
bytedcli lark im +messages-search --query "" --chat-id oc_xxx --sender ou_me --start "2026-03-18T00:00:00+08:00" --end "2026-03-25T23:59:59+08:00" --page-size 50 --page-limit 5 --format json

# If the bounded run still returns has_more=true, continue manually
bytedcli lark im +messages-search --query "" --chat-id oc_xxx --sender ou_me --start "2026-03-18T00:00:00+08:00" --end "2026-03-25T23:59:59+08:00" --page-size 50 --page-token <token_from_previous_run> --format json
```
