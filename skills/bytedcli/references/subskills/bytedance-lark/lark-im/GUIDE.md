...
Shortcut 是对常用操作的高级封装（`bytedcli lark im +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema im.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark im <resource> <method> [flags] # 调用 API
```
