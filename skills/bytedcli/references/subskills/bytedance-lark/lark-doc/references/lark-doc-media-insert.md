...
```bash
# 🟢 推荐：从剪切板直接插入（无需先存盘）
bytedcli lark docs +media-insert --doc doxcnXXX --from-clipboard
...
# 插入图片（默认）
bytedcli lark docs +media-insert --doc doxcnXXX --file ./image.png

# doc 支持直接传 docx URL（自动提取 document_id）
bytedcli lark docs +media-insert --doc "https://xxx.feishu.cn/docx/doxcnXXX" --from-clipboard

# 如果上一步是 create-doc，优先传返回值里的 doc_id
# 不要把 /wiki/... 形式的 doc_url 直接传给 docs +media-insert
bytedcli lark docs +media-insert --doc doxcnReturnedByCreateDoc --file ./image.png

# 插入文件（非图片）
bytedcli lark docs +media-insert --doc doxcnXXX --file ./spec.pdf --type file

# 图片对齐与描述（caption）
bytedcli lark docs +media-insert --doc doxcnXXX --from-clipboard --align center --caption "架构图"
```
