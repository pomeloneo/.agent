...
> 正确：`bytedcli lark docs +search --query "方案"`
> 错误：`bytedcli lark docs +search 方案`
> `+search` 不接受“搜索词位置参数”这种写法；如果把关键词直接跟在命令后面，不会进入 `query`，会变成空搜或返回不符合预期的结果。

```bash
# 关键词搜索
bytedcli lark docs +search --query "季度总结"

# 搜标题里带“评测结果”的电子表格 / 文档
bytedcli lark docs +search --query "评测结果"

# 标题包含关键词（默认按关键词检索，不做精确标题匹配）
bytedcli lark docs +search --query "方案"

# 使用服务端标题限定语法
bytedcli lark docs +search --query 'intitle:方案'

# 精确短语匹配
bytedcli lark docs +search --query '"季度 总结"'

# 逻辑或 / 排除
bytedcli lark docs +search --query '方案 OR 草稿'
bytedcli lark docs +search --query '方案 -草稿'

# 标题精确短语匹配
bytedcli lark docs +search --query 'intitle:"季度总结"'

# 按最近打开时间过滤
bytedcli lark docs +search \
  --query "方案" \
...
# 人类可读格式输出
bytedcli lark docs +search --query "OKR" --format pretty

# 返回原始 JSON，并用 page_token 翻页
bytedcli lark docs +search --query "方案" --format json
bytedcli lark docs +search --query "方案" --format json --page-token '<PAGE_TOKEN>'
```
...
- 参数传递：只要用户给了搜索关键词，就必须显式使用 `--query "<关键词>"`。不要生成 `bytedcli lark docs +search 方案`、`bytedcli lark docs +search xxx（搜索关键词）` 这种位置参数写法。
