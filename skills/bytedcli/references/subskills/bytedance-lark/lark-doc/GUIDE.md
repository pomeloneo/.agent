...
```bash
# 常用示例
bytedcli lark docs +fetch  --api-version v2 --doc "文档URL或token"
bytedcli lark docs +create --api-version v2 --content '<title>标题</title><p>内容</p>'
bytedcli lark docs +update --api-version v2 --doc "文档URL或token" --command append --content '<p>内容</p>'
```
...
- 用户说"看一下文档里的图片/附件/素材""预览素材" → 用 `bytedcli lark docs +media-preview`
- 用户明确说"下载素材" → 用 `bytedcli lark docs +media-download`
- 如果目标是画板/whiteboard/画板缩略图 → 只能用 `bytedcli lark docs +media-download --type whiteboard`（不要用 `+media-preview`）
- 用户说"找一个表格""按名称搜电子表格""找报表""最近打开的表格""最近我编辑过的 xxx" → 直接用 `bytedcli lark drive +search`（参考 [`lark-drive`](../lark-drive/references/lark-drive-search.md)）。**老的 `docs +search` 已进入维护期、后续会下线，不要再新增依赖。**
...
Shortcut 是对常用操作的高级封装（`bytedcli lark docs +<verb> [flags]`）。有 Shortcut 的操作优先使用。
