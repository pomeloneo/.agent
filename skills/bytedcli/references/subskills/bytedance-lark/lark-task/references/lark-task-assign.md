...
```bash
# Add an assignee
bytedcli lark task +assign --task-id "<task_guid>" --add "ou_aaa"

# Transfer an assignee (remove old, add new)
bytedcli lark task +assign --task-id "<task_guid>" --remove "ou_old" --add "ou_new"

# Add multiple assignees
bytedcli lark task +assign --task-id "<task_guid>" --add "ou_aaa,ou_bbb"
```
