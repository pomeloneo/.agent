...
```bash
# Update task summary
bytedcli lark task +update --task-id "<task_guid>" --summary "New Summary"

# Update multiple tasks' due dates
bytedcli lark task +update --task-id "<task_guid>,<another_task_guid>" --due "+2d"

# Update with JSON data
bytedcli lark task +update --task-id "<task_guid>" --data '{"description": "New description"}'
```
...
2. Execute `bytedcli lark task +update --task-id "..." ...`
