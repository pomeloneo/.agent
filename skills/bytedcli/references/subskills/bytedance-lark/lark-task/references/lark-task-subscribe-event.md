...
```bash
bytedcli lark event +subscribe --event-types task.task.update_user_access_v2 --compact --quiet
```

The full flow is:
1. Register the subscription with `bytedcli lark task +subscribe-event [--as user|bot]`
2. Receive those events with `bytedcli lark event +subscribe --event-types task.task.update_user_access_v2 ...`

## Recommended Commands

```bash
bytedcli lark task +subscribe-event
```
# Subscribe with app identity
bytedcli lark task +subscribe-event --as bot
...
2. Execute `bytedcli lark task +subscribe-event`
