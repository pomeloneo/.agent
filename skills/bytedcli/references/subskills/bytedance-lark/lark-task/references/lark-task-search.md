...
```bash
# Search by keyword
bytedcli lark task +search --query "test"

# Search incomplete tasks assigned to specific users
bytedcli lark task +search --assignee "ou_xxx,ou_yyy" --completed=false

# Search by due time range
bytedcli lark task +search --query "release" --due "-1d,+7d"
```
...
2. Execute `bytedcli lark task +search ...`
