...
```bash
bytedcli lark slides xml_presentation.slide replace --as user --params '<json_params>' --data '<json_data>'
```
...
```bash
bytedcli lark slides xml_presentation.slide replace --as user --params '{
  "xml_presentation_id": "slides_example_presentation_id",
  "slide_id": "slide_example_id"
}' --data '{
  "parts": [
...
```bash
# 先拿 file_token
TOKEN=$(bytedcli lark slides +media-upload --file ./pic.png --presentation "$PID" --as user | jq -r '.data.file_token')
...
7. **执行前必做**：`bytedcli lark schema slides.xml_presentation.slide.replace` 查看最新参数结构。
