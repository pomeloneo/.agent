...
bytedcli lark slides +media-upload --as user \
  --file ./pic.png \
...
# 预览（不实际上传）
bytedcli lark slides +media-upload --file ./pic.png --presentation $PRES_ID --dry-run
```
...
| 1061004 | forbidden：当前身份对该演示文稿无编辑权限 | 确认当前身份（user 或 bot）对目标 PPT 有编辑权限。bot 模式常见原因：PPT 不是该 bot 创建的——可用 `+create --as bot` 新建，或以 user 身份给 bot 授权 `bytedcli lark drive permission.members create --as user ...` |
