...
```bash
bytedcli lark slides xml_presentations get --as user --params '<json_params>'
```
...
```bash
bytedcli lark slides xml_presentations get --as user --params '{"xml_presentation_id":"slides_example_presentation_id"}'
```
...
```bash
bytedcli lark slides xml_presentations get --as user --params '{"xml_presentation_id":"slides_example_presentation_id"}' > presentation_data.json
```
...
1. **执行前必做**: 使用 `bytedcli lark schema slides.xml_presentations.get` 查看最新的参数结构
