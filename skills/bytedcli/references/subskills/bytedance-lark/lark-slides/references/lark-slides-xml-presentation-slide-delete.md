...
```bash
bytedcli lark slides xml_presentation.slide delete --as user --params '<json_params>'
```
...
```bash
# 先读取 XML 内容，确认待删除页面
bytedcli lark slides xml_presentations get --as user --params '{"xml_presentation_id":"slides_example_presentation_id"}' | jq -r '.data.xml_presentation.content'

# 然后按已知 slide_id 删除
bytedcli lark slides xml_presentation.slide delete --as user --params '{"xml_presentation_id":"slides_example_presentation_id","slide_id":"slide_example_id"}'
```
...
1. **执行前必做**: 使用 `bytedcli lark schema slides.xml_presentation.slide.delete` 查看最新的参数结构
...
```bash
bytedcli lark slides xml_presentation.slide create --as user --params '{"xml_presentation_id":"slides_example_presentation_id"}' --data '{
  "slide": {
...
for slide_id in sld_a sld_b sld_c; do
  bytedcli lark slides xml_presentation.slide delete --as user --params "{\"xml_presentation_id\":\"slides_example_presentation_id\",\"slide_id\":\"$slide_id\"}"
done
```
