...
# 创建 PPT + 添加 slide 页面
bytedcli lark slides +create --title "项目汇报" --slides '[
...
# 以应用身份创建（自动授权当前用户）
bytedcli lark slides +create --title "项目汇报" --as bot

# 预览（不执行）
bytedcli lark slides +create --title "项目汇报" --slides '[...]' --dry-run
```
...
>
> 如果演示文稿是**以应用身份（bot）创建**的，如 `bytedcli lark slides +create --as bot`，CLI 会**尝试为当前 CLI 用户自动授予该演示文稿的 `full_access`（可管理权限）**。
>
...
```bash
bytedcli lark slides +create --as user --title "图测试" --slides '[
...
```bash
# 1) 上传图片
TOKEN=$(bytedcli lark slides +media-upload --as user \
...
```bash
# 第 1 步：创建空白 PPT
PRES_ID=$(bytedcli lark slides +create --title "项目汇报" | jq -r '.data.xml_presentation_id')
