...
```bash
bytedcli lark slides +create --title "项目汇报"
```
...
```bash
PRESENTATION_ID=$(bytedcli lark slides +create --title "季度复盘" | jq -r '.data.xml_presentation_id')

bytedcli lark slides xml_presentation.slide create --as user --params "{\"xml_presentation_id\":\"$PRESENTATION_ID\"}" --data '{
  "slide": {
...
```bash
bytedcli lark slides xml_presentations get --as user --params '{
  "xml_presentation_id": "slides_example_presentation_id"
}'
```
...
```bash
bytedcli lark slides xml_presentation.slide create --as user --params '{
  "xml_presentation_id": "slides_example_presentation_id"
}' --data '{
  "slide": {
...
```bash
bytedcli lark slides xml_presentation.slide delete --as user --params '{
  "xml_presentation_id": "slides_example_presentation_id",
  "slide_id": "slide_example_id"
}'
```
...
```bash
PRESENTATION_ID=$(bytedcli lark slides +create --title "从文件添加页面" | jq -r '.data.xml_presentation_id')
```

再用 `jq` 组装请求体，从文件添加页面：

```bash
bytedcli lark slides xml_presentation.slide create --as user \
...
# 1. 上传图片拿 file_token
TOKEN=$(bytedcli lark slides +media-upload --file ./pic.png --presentation "$PID" --as user \
  | jq -r '.data.file_token')

# 2. block_insert 到页面末尾（省略 insert_before_block_id）
# 注：<img .../> 是自闭合标签，CLI 不会展开（只有 <shape/> 会被补 <content/>）
bytedcli lark slides +replace-slide --as user \
...
for slide_xml in "${slides[@]}"; do
  ...
  bytedcli lark slides xml_presentation.slide create --as user --params "{\"xml_presentation_id\":\"$PRESENTATION_ID\"}" --data "$payload"
done
```
