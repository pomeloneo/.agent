...
```bash
bytedcli lark auth login --domain slides
```
...
```bash
bytedcli lark slides +create --title "演示文稿标题" --slides '[
...
```bash
bytedcli lark slides xml_presentations get --as user \
...
```bash
# 创建
bytedcli lark slides +create --as user --title "Demo" --slides '[...]'
...
```bash
# 追加到末尾
bytedcli lark slides xml_presentation.slide create \
  --as user \
  --params '{"xml_presentation_id":"YOUR_ID"}' \
...
1. **使用 `wiki.spaces.get_node` 查询节点信息**
   ```bash
   bytedcli lark wiki spaces get_node --as user --params '{"token":"wiki_token"}'
   ```
...
```bash
# 查询 wiki 节点
bytedcli lark wiki spaces get_node --as user --params '{"token":"wikcnxxxxxxxxx"}'
```
...
```bash
# 用 obj_token 读取幻灯片内容
bytedcli lark slides xml_presentations get --as user --params '{"xml_presentation_id":"xxxxxxxxxxxx"}'
```
...
Shortcut 是对常用操作的高级封装（`bytedcli lark slides +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema slides.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark slides <resource> <method> [flags] # 调用 API
```
...
> **注意**：如果 md 内容与 `slides_xml_schema_definition.xml` 或 `bytedcli lark schema slides.<resource>.<method>` 输出不一致，以后两者为准。
