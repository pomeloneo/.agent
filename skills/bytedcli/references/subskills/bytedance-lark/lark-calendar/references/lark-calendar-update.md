...
如需更新 `location`（地理位置，不含会议室位置）、`visibility`（日程公开范围）、自定义 `reminders`（提醒设置）、自定义 `attendee_ability`（参与人权限）、自定义 `free_busy_status`（日程忙闲状态）、`color`（颜色）、附件、视频会议信息、全天日程，或在新增参会人时配置可选参加状态 等高级参数，请改用完整的 API 命令。建议先通过 `bytedcli lark schema calendar.events.patch`、`bytedcli lark schema calendar.event.attendees.create`、`bytedcli lark schema calendar.event.attendees.batch_delete` 查看完整参数定义。
