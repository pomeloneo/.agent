...
# 指定日历
bytedcli lark calendar +create --summary "..." --start "..." --end "..." \
  --calendar-id cal_xxx
```
...
# 第一步：创建日程（含高级参数）
## 查看完整参数定义
bytedcli lark schema calendar.events.create
## 创建日程
bytedcli lark calendar events create --calendar-id primary --data '{
  "summary": "产品评审",
...
## 查看完整参数定义
bytedcli lark schema calendar.event.attendees.create
## 添加参会人
bytedcli lark calendar event.attendees create \
  --calendar-id <CALENDAR_ID> --event-id <EVENT_ID> \
  --data '{"attendees": [{"type": "user", "user_id": "ou_xxx"}]}'

# 可选第三步（推荐）：若第二步失败，回滚删除空日程
## 查看完整参数定义
bytedcli lark schema calendar.events.delete
## 删除空日程
bytedcli lark calendar events delete \
  --calendar-id <CALENDAR_ID> --event-id <EVENT_ID> \
