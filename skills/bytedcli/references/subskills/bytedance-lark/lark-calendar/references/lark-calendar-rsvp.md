...
```bash
# 回复日程为接受 (使用主日历)
bytedcli lark calendar +rsvp --event-id evt_xxx --rsvp-status accept

# 回复日程为拒绝
bytedcli lark calendar +rsvp --event-id evt_xxx --rsvp-status decline

# 回复日程为待定
bytedcli lark calendar +rsvp --event-id evt_xxx --rsvp-status tentative

# 指定其他日历下的日程
bytedcli lark calendar +rsvp --calendar-id cal_xxx --event-id evt_xxx --rsvp-status accept
```
