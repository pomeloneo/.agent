...
**CRITICAL — 会议与日程的意图路由：**
- **查询过去时间的会议**：如果用户明确查询过去时间的会议（如“昨天的会议”、“上周的会议”），**优先使用 [`../lark-vc/GUIDE.md`](../lark-vc/GUIDE.md) 搜索会议记录**。因为会议数据不仅包含从日程发起的视频会议，还包含即时会议，仅查询日程数据会导致结果不全。
...
Shortcut 是对常用操作的高级封装（`bytedcli lark calendar +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
bytedcli lark schema calendar.<resource>.<method>   # 调用 API 前必须先查看参数结构
bytedcli lark calendar <resource> <method> [flags] # 调用 API
```
