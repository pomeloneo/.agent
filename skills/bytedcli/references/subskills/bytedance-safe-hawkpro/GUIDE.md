...
description: Hawkpro trace and error-reason query operations via bytedcli safe domain. Use when the user asks for an objectID's machine-review error reason or Hawkpro error reason; also use when tracing moderation decisions by user/room/time/hit boolean filter and viewing decision graph or rule-hit details.
---
