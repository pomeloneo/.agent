---
name: bytedance-safe-bcp
description: BCP reconciliation key list queries via bytedcli safe domain. Use when the user wants to inspect bcp_key reconciliation rows for one rule and time window, including success/failure/miss-param/exec-fail/build-fail drilldown and feature_list payload. Backed by the Aeolus dashboard 263849. Use for 中文意图：BCP对账、bcp_key 列表、对账规则、规则id、对账case、对账成功、对账失败、对账一致、对账不一致、对账异常、对账缺参、对账执行失败、对账构建失败、BCPResultCode、业务对账、数据对账、对账时间、对账时间窗口、最近N分钟对账、Aeolus 看板、reconciliation.
---
