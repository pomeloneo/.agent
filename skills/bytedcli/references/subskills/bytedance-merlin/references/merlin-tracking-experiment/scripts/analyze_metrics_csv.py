...
_LOWER_BETTER_RE = re.compile(
    r"(loss|error|wer|cer|ppl|perplex|rmse|mae|mse|nll)", re.IGNORECASE
)
_HIGHER_BETTER_RE = re.compile(
    r"(acc|accuracy|auc|f1|precision|recall|map|ndcg|bleu|rouge)", re.IGNORECASE
)
...
def _pick_x_column(
    fieldnames: Sequence[str], explicit_x: Optional[str]
) -> Optional[str]:
    if explicit_x:
        return explicit_x
...
def _summarize_series(
    series: Series, direction: Optional[str], recent_window: int
) -> Dict[str, object]:
...
def _load_run(
    ...
    for idx, row in enumerate(reader):
        ...
        if wanted_metrics is None:
            ...
            candidate_fields = [
                f for f in reader.fieldnames if f in wanted_metrics and f != chosen_x
            ]
...
def _print_text_summary(
    ...
    for run in runs:
        ...
        if incremental_from_x is not None:
            ...
            print(
                f"INCREMENTAL_FROM_X: {_format_float(last_x) if last_x is not None else '-'}"
            )
        ...
        for metric_name in sorted(run.metrics.keys()):
            ...
            summary = _summarize_series(
                Series(series.x, ys_s), direction=direction, recent_window=recent_window
            )
            ...
            if mean is not None and stdev is not None:
                parts.append(
                    f"recent_mean±std={_format_float(mean)}±{_format_float(stdev)}"
                )
            if direction:
...
def _build_html_report(
    ...
    for run in runs:
        ...
        for metric_name, series in run.metrics.items():
            if metric_filter:
                if (
                    metric_name not in metric_filter
                    and _normalize_header(metric_name) not in metric_filter
                ):
                    continue
            ys = series.y
            if ema_alpha is not None:
        ...
        run_payload.append(
            {"name": run.name, "x_name": run.x_name, "series": series_payload}
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Analyze Merlin metrics CSV and plot curves (no extra deps)."
    )
    ...
    parser.add_argument(
        ...
        dest="x_name",
        default=None,
        help="x-axis column name (default: auto-detect)",
    )
    parser.add_argument(
        "--metrics",
        nargs="*",
        default=None,
        help="metric column names to analyze/plot (default: auto)",
    )
    ...
    parser.add_argument(
        "--limit", type=int, default=None, help="max rows to read per CSV"
    )

    smooth_group = parser.add_mutually_exclusive_group()
    smooth_group.add_argument(
        "--smooth", type=int, default=0, help="moving average window (default: 0)"
    )
    smooth_group.add_argument(
        "--ema", type=float, default=None, help="EMA alpha in (0,1)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="recent window for trend stats (default: 20)",
    )
    parser.add_argument(
        "--state-dir",
        default=None,
        help="state directory for incremental monitoring (optional)",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="disable incremental mode and analyze full CSV",
    )
    parser.add_argument("--out", default=None, help="output HTML path (optional)")
    parser.add_argument(
        "--title", default="Metrics Curves Report", help="HTML report title"
    )
    parser.add_argument(
        "--json",
        dest="json_out",
            ...
            for metric_name, series in run.metrics.items():
                ...
                ys_s = (
                    ...
                    if args.ema is not None
                        ...
                        _moving_average(ys, args.smooth)
                        if args.smooth > 1
                        else list(ys)
                    )
                )
                metrics_payload[metric_name] = _summarize_series(
                    Series(series.x, ys_s),
                    direction=direction,
                    recent_window=max(args.recent_window, 3),
                )
            payload.append(
                {"run": run.name, "x": run.x_name, "metrics": metrics_payload}
            )
