...
def analyze_csv(file_path, window_size=10):
    ...
    if total_points > recent_window:
        ...
        print(
            f"Trend (last {recent_window} vs prev {recent_window}): {trend} ({percent_change:.2f}%)"
        )
    ...
    if len(recent_data) > 1:
        ...
        if abs(z_score) > 3:
            print(
                f"WARNING: Potential Anomaly Detected! Last point is {z_score:.2f} std devs from recent mean."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--window", type=int, default=20, help="Window size for trend analysis"
    )
