...
def cmd_concat(args: argparse.Namespace) -> int:
    ...
    out_fieldnames = [args.run_column] + [
        f for f in all_fieldnames if f != args.run_column
    ]
    _write_rows(
        Path(args.out),
        rows_out,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Small CSV utilities for metrics analysis."
    )
    ...
    p_select.add_argument(
        "--columns", required=True, help="comma separated column names to keep"
    )
    p_select.add_argument("--out", required=True, help="output CSV path")
    p_select.set_defaults(func=cmd_select)

    p_concat = sub.add_parser(
        "concat", help="concat multiple CSVs and add a run column"
    )
    p_concat.add_argument("inputs", nargs="+", help="input CSV paths")
    p_concat.add_argument(
        "--run-column", default="run", help="run column name (default: run)"
    )
