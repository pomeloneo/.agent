...
def _run_merlin_cli(args: Sequence[str], retries: int = 0) -> Dict[str, Any]:
    cmd = ["merlin-cli", *args]
    ...
    for attempt in range(1 + retries):
        if attempt > 0:
            ...
            print(
                f"[retry {attempt}/{retries}] waiting {wait:.0f}s before retrying …",
                file=sys.stderr,
            )
            time.sleep(wait)
        try:
            ...
            last_err = e
            stderr_snippet = getattr(e, "__str__", lambda: "")()
            if (
                "502" in stderr_snippet
                or "Bad Gateway" in stderr_snippet
                or "upstream" in stderr_snippet
            ):
                continue
            if attempt < retries:
                continue
...
def _extract_exercises(arena_eval: Dict[str, Any]) -> List[ExerciseRow]:
    detail = _get_nested(
        arena_eval, ["arena_evaluation", "progress", "detail"]
    ) or _get_nested(arena_eval, ["evaluation", "progress", "detail"])
    if not isinstance(detail, dict) or not detail:
        raise RuntimeError(
            "missing arena_evaluation.progress.detail in get_arena_evaluation output"
        )
    ...
    for exercise_key, v in detail.items():
        ...
        avg_score = (
            _as_float(score.get("avg_score")) if isinstance(score, dict) else None
        )
        error_rate = (
            _as_float(score.get("error_rate")) if isinstance(score, dict) else None
        )
...
def _write_exercises_csv(rows: List[ExerciseRow], out_path: Path) -> None:
    ...
    score_keys = [k for k in preferred if k in score_keys] + [
        k for k in score_keys if k not in preferred
    ]
    fieldnames = [
        "exercise_key",
        "exercise_sid",
...
def _extract_case_brief(case_item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    predict = (
        payload_obj.get("predict0")
        or payload_obj.get("predict_0")
        or payload_obj.get("predict")
    )
    answer = payload_obj.get("answer")
    traj = (
        payload_obj.get("__trajectory_url___0")
        or payload_obj.get("trajectory_url")
        or payload_obj.get("__trajectory_url__")
    )
    question_id = (
    ...
    return {
        ...
        "trajectory_url": traj,
        "payload_keys": (
            sorted(list(payload_obj.keys())) if isinstance(payload_obj, dict) else []
        ),
    }
...
def _fetch_cases_for_exercise(
    ...
    wrote = 0
    page = 0
    target_label = exercise.exercise_key or exercise.exercise_version_sid
    with out_path.open("w", encoding="utf-8") as f:
        while wrote < want:
            page += 1
            limit = min(page_size, want - wrote)
            print(
                f"\r  {target_label}: fetching page {page} (offset={offset}, {wrote} cases so far) …",
                end="",
                flush=True,
                file=sys.stderr,
            )
            ...
            if len(items) < limit:
                break
    print(f"\r  {target_label}: done — {wrote} cases saved{' ' * 40}", file=sys.stderr)
...
def _write_report(
    ...
    ev = arena_eval.get("arena_evaluation") or arena_eval.get("evaluation") or {}
    ...
    lines.append("")
    lines.append(
        "| # | exercise_key | exercise_version_sid | completed | avg_score | error_rate |"
    )
    lines.append("|---:|---|---|---:|---:|---:|")
    ...
    if case_stats:
        ...
        for ex, n, p in case_stats:
            ...
            lines.append(
                f"| {ex.exercise_version_sid} | {avg_score_cell} | {n} | {file_cell} |"
            )
        lines.append("")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument(
        "--fetch-cases", action="store_true", help="also fetch cases per exercise"
    )
    p.add_argument(
        "--exercise-version-sid",
        default=None,
        help="only fetch cases for this exercise_version_sid",
    )
    p.add_argument(
        "--max-exercises-for-cases",
        type=int,
        default=10,
        help="limit exercises when fetching cases (0 = no limit)",
    )
    p.add_argument(
        "--cases-per-exercise",
        type=int,
        default=50,
        help="max cases per exercise (0 = no limit)",
    )
    p.add_argument(
        "--page-size", type=int, default=100, help="page size for arena list-case"
    )
    ...
    arena_eval = _run_merlin_cli(
        [
            ...
            "get-evaluation",
            "--json",
            json.dumps({"sid": evaluation_instance_sid}),
        ]
    )
    raw_path = out_dir / "arena_evaluation.raw.json"
    raw_path.write_text(
        json.dumps(arena_eval, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    ...
    if args.fetch_cases:
        ...
        total = len(chosen)
        print(f"Fetching cases for {total} exercise(s) …", file=sys.stderr)
        for i, ex in enumerate(chosen, 1):
            print(
                f"[{i}/{total}] {ex.exercise_key or ex.exercise_version_sid}",
                file=sys.stderr,
            )
            try:
