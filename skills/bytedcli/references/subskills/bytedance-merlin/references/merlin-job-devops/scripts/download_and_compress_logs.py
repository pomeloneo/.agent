...
_ERROR_RE = re.compile(
    r"\b(error|exception|traceback|fatal|panic|segfault|signal)\b", re.IGNORECASE
)
_WARN_RE = re.compile(r"\b(warn|warning)\b", re.IGNORECASE)
_OOM_RE = re.compile(
    r"\b(oom|out of memory|cuda.*out of memory|cudnn|nccl|killed|sigkill)\b",
    re.IGNORECASE,
)
_PROGRESS_RE = re.compile(
    r"\b(epoch|step|iter|iteration|global_step|progress|eta)\b", re.IGNORECASE
)
_METRIC_RE = re.compile(
    r"\b(loss|acc|accuracy|eval|val|precision|recall|f1|auc|lr|learning rate)\b",
    re.IGNORECASE,
)
...
def _download_text(
    url: str, timeout_s: int, max_bytes: int, disable_proxy: bool
) -> str:
...
def _ensure_pairs(args: Sequence[str]) -> List[Tuple[str, str]]:
    if len(args) == 0 or len(args) % 2 != 0:
        raise RuntimeError(
            "USAGE: download_and_compress_logs.py JOB1 URL1 [JOB2 URL2 ...]"
        )
...
def _compute_delta(
    text: str, prev: Optional[IncrementalState]
) -> Tuple[str, bool, int]:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Download job logs and produce a compressed snippet."
    )
    parser.add_argument(
        "pairs", nargs="+", help="JOB URL pairs: JOB1 URL1 [JOB2 URL2 ...]"
    )
    parser.add_argument(
        "--out-dir", default="./logs", help="directory to save logs (default: ./logs)"
    )
    parser.add_argument(
        "--state-dir",
        default=None,
        help="state directory for incremental monitoring (default: <out-dir>/.state)",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="disable incremental mode and process full log",
    )
    parser.add_argument(
        "--timeout", type=int, default=30, help="download timeout seconds (default: 30)"
    )
    parser.add_argument(
        "--max-bytes",
        type=int,
        default=50 * 1024 * 1024,
        help="max bytes per log (default: 50MB)",
    )

    parser.add_argument(
        "--head", type=int, default=60, help="keep first N lines (default: 60)"
    )
    parser.add_argument(
        "--tail", type=int, default=160, help="keep last N lines (default: 160)"
    )
    parser.add_argument(
        "--context",
        type=int,
        default=2,
        help="context lines around key lines (default: 2)",
    )
    parser.add_argument(
        "--max-lines",
        type=int,
        default=400,
        help="max lines in compressed snippet (default: 400)",
    )
    parser.add_argument(
        "--anchor-bytes",
        type=int,
        default=4096,
        help="anchor bytes kept in state (default: 4096)",
    )
    parser.add_argument(
        "--keep-ansi", action="store_true", help="keep ANSI escapes (default: strip)"
    )
    parser.add_argument(
        "--use-proxy",
        action="store_true",
        help="use proxy env vars (default: disabled)",
    )
    ...
    for job, url in pairs:
        ...
        try:
            ...
            compressed_text, original_lines, compressed_lines = _compress_text(
                text, cfg=cfg
            )
            delta_compressed_text, delta_original_lines, delta_compressed_lines = (
                _compress_text(delta_text, cfg=cfg)
            )
