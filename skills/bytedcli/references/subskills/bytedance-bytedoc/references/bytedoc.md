...
- `bytedoc access role list` 会读取数据库静态信息、IAM resource info 和角色绑定，输出可供 Agent 展示给用户选择的角色列表；支持 `--db-name`、`--service`、`--backend` 消歧。classic / cloud-native 返回 `data.result.kind=classic_role_list`，字段包含 `roles[]` / `recommendedCommands`；显式 `--backend volc` 时兼容路由到 Volc PSM 授权巡检，返回 `data.result.kind=volc_psm_inspection`，字段包含 `inspection` / `items[]` / `accountCheck`，并在传入 classic IAM 专属 flag 时通过 `warnings[]` 标明已忽略。
- `bytedoc access permission get` 会校验当前用户是否具备发起授权流程的前置权限。classic / cloud-native 返回 `data.result.kind=classic_permission_preflight`，字段包含 `selectedRole` / `canApplyFromCurrentUser` / `nextActions`；显式 `--backend volc` 时不会走 classic IAM preflight，而是兼容路由到 Volc PSM 授权巡检，`--role-name`、`--role-names`、`--env` 在该路径下不参与检查。
